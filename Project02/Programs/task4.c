...
/*
function takes a defined category and return 1 if the c is in the category
*/
...
char* fullSort(char* s){
	...
	Category number = {0,1,"number","0","9"};
	Category operator = {1,7,"operatory","+-*/=!%","+-*/=!%"};
	Category letter = {2,2,"letter","aA","zZ"};

	// add category stuct to the array based on its unique index
	cate.allCate[number.index]=number;
	cate.allCate[operator.index]=operator;
