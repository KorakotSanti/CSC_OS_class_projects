...
#define TEXTFILE "robotSimulOut.txt"
FILE* outText; //open the ouput text file to write in
...
pthread_mutex_t** gridLock; //lock for each individual tile on the grid
pthread_mutex_t fileLock; //lock for writing into file
...
void initializeApplication(void)
{
	...
	for (int i=0; i<numRows; i++){
		...
		gridLock[i] = (pthread_mutex_t*) malloc(numCols*sizeof(pthread_mutex_t));
	}
	...
	// initialize lock for each grid space
	for(int i=0;i<numRows;i++){
		for(int j=0;j<numCols;j++){
			pthread_mutex_init(&gridLock[i][j],NULL);
		}
	}
	// initialize mutex lock for locking file access
	pthread_mutex_init(&fileLock,NULL);
	// pthread variable for number of robot and boxes pair
	pthread_t robotthr[numBoxes];

	// add all the initial robot, box, and door in random position on grid
	addrobotboxdoor();

	writeInitialPositions();
	// cout<<numBoxes<<endl;
	for (int i = 0;i<numBoxes;i++){
		roboxInfo* bot = &robox[i];
		int err = pthread_create(&robotthr[i],NULL,robotfunc,(void*)bot);
		if(err!=0){
			cout<<"Cound not create Thread"<<endl;
			exit(1);
		}
	}
...
void getRoboxInfo(int i){
	...
	// lock the grid space for the robot and bot intial placement
	pthread_mutex_lock(&gridLock[boxrow][boxcol]);
	pthread_mutex_lock(&gridLock[robotrow][robotcol]);
...
void* robotfunc(void* botstruct){
	// create an vector of movelist
	roboxInfo* bot = (roboxInfo*)botstruct;
	...
	bot->stat=ENDCOM;
	pthread_mutex_unlock(&gridLock[bot->rrow][bot->rcol]);
	pthread_mutex_unlock(&gridLock[bot->brow][bot->bcol]);
	writeRobotexe(bot);
	numLiveThreads--;
	return NULL;
}
	...
	while (true){
		...
		switch(bot->dir){
			case NORTH:
					...
					bot->dir=NORTH;
					//move robot
					pthread_mutex_lock(&gridLock[bot->rrow-1][bot->rcol]);
					bot->rrow-=1;
					pthread_mutex_unlock(&gridLock[bot->rrow+1][bot->rcol]);
					writeRobotexe(bot);
			...
			case EAST:
					...
					usleep(100000);
					bot->dir = EAST;
					pthread_mutex_lock(&gridLock[bot->rrow][bot->rcol+1]);
					bot->rcol+=1;
					pthread_mutex_unlock(&gridLock[bot->rrow][bot->rcol-1]);
					writeRobotexe(bot);
...
void pushcommand(roboxInfo* bot){
	...
	switch(bot->dir){
		case NORTH:
				...
				while(bot->rowmove != 0){
					usleep(100000);
					pthread_mutex_lock(&gridLock[bot->brow-1][bot->bcol]);
					bot->brow-=1;
					pthread_mutex_unlock(&gridLock[bot->brow+1][bot->bcol]);
		...
		case EAST:
				while(bot->colmove != 0){
					usleep(100000);
					pthread_mutex_lock(&gridLock[bot->brow][bot->bcol+1]);
					bot->bcol+=1;
					pthread_mutex_unlock(&gridLock[bot->brow][bot->bcol-1]);
